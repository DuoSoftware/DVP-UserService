...
function addAuditTrail(tenantId, companyId, iss, auditData) {
  ...
  try {
    auditTrailsHandler.CreateAuditTrails(
      tenantId,
      companyId,
      iss,
      auditData,
      function(err, obj) {
        if (err) {
          ...
          logger.error(
            "addAuditTrail -  Fail To Save Audit trail-[%s]",
            jsonString
          );
        }
      }
...
function GetUserCount(req, res) {
  ...
  UserAccount.count(queryString).exec(function(err, resCount) {
    if (err) {
      jsonString = messageFormatter.FormatMessage(
        err,
        "Get Users  failed",
  ...
  UserAccount.findOne(query)
    ...
    .exec(function(err, userAccounts) {
      if (err) {
        ...
        var users = userAccounts.reduce((acc, userAccount) => {
          if (userAccount.userref) {
...
function GetUsersByRoles(req, res) {
  ...
  if (isPaging) {
    UserAccount.find(qObj)
      .select("-user_scopes -client_scopes")
      .populate("userref", "-password -user_scopes -client_scopes")
      .skip(skip)
      .limit(size)
      .exec(execFunc);
  } else {
  ...
  User.find({
    ...
    if (err) {
      ...
      if (users) {
        ...
        checkInvitedUsers(unavailableUsers, company, tenant, function(
          errInv,
          resInv
        ) {
          invitedUsers = resInv;
          ...
          UserAccount.find({
            ...
            .exec(function(err, userAccounts) {
              if (err) {
                jsonString = messageFormatter.FormatMessage(
                  err,
                  "Get User Account Failed, Not available for invitation",
                  false,
                  undefined
                );
              } else {
                if (userAccounts) {
                  ...
                  var unavailableAccounts = arr_diff(
                    commonUsers,
                    availableAccounts
                  );
                  ...
                  jsonString = messageFormatter.FormatMessage(
                    err,
                    "Get User Account Successful, Not available for invitation",
                    true,
                    {
                  ...
                  jsonString = messageFormatter.FormatMessage(
                    err,
                    "No user Account found, Available for Invitation",
                    true,
                    {
                      requestUsers: userNames,
                      unavailableAccounts: unavailableAccounts,
...
function checkInvitedUsers(userNames, company, tenant, callback) {
  UserInvitation.find({
    ...
    company: company,
    tenant: tenant
  }).exec(function(err, resInv) {
    if (err) {
      logger.debug(
        "DVP-UserService.UserInvitable Searching invitations failed "
      );
      callback(err, []);
    } else {
      if (resInv) {
        var invUsers = resInv.map(function(item) {
          return item.to;
        });
        callback(undefined, invUsers);
      } else {
        callback(undefined, []);
      }
    }
  ...
  Org.findOne({ tenant: tenant, id: company }, function(err, org) {
    if (err) {
      ...
      if (org.ownerId == req.params.name) {
        jsonString = messageFormatter.FormatMessage(
          undefined,
          "Delete organization owner failed",
          false,
          undefined
        );
        console.log(jsonString);
            ...
            if (err) {
              ...
              Org.findOne({ tenant: tenant, id: company }, function(err, org) {
                if (err) {
                  ...
                  var limitObj = FilterObjFromArray(
                    org.consoleAccessLimits,
                    "accessType",
                    user.user_meta.role
                  );
                  if (limitObj) {
                    ...
                    if (userIndex > -1) {
                      ...
                      Org.findOneAndUpdate(
                        { id: company, tenant: tenant },
                        org,
                        function(err, rOrg) {
                          if (err) {
                            jsonString = messageFormatter.FormatMessage(
                              err,
                              "Update Console Access Limit Failed",
                              false,
                              undefined
                            );
                            console.log(jsonString);
                            ...
                            jsonString = messageFormatter.FormatMessage(
                              err,
                              "Update Console Access Limit Success",
                              true,
                              undefined
                            );
                            console.log(jsonString);
                    ...
                    console.log(
                      "Failed to update currentAccess, Cannot find Org accessType"
                    );
                  }
                }
              });
                ...
                .exec(function(err, userAccount) {
                  if (err) {
                    ...
                    if (userAccount) {
                      ...
                      User.findOne({ username: req.body.mail })
                        .select("-password")
                        .exec(function(err, user) {
                          if (err) {
                            jsonString = messageFormatter.FormatMessage(
                            ...
                            if (user) {
                              jsonString = messageFormatter.FormatMessage(
                                err,
                                "User Already Exists, Please send an invitation",
                                false,
                                undefined
                              );
                              res.end(jsonString);
                                      ...
                                      UserAccount.findOneAndUpdate(
                                          ...
                                          function(err, rUsers) {
                                            if (err) {
                                              ...
                                              jsonString = messageFormatter.FormatMessage(
                                                  undefined,
                                                  "Update user scope successfully",
                                                  true,
                                                  undefined
                                              );
                                            }
                                      ...
                                      Org.findOneAndUpdate(
                                        ...
                                        function(err, rOrg) {
                                          if (err) {
                                            ...
                                            jsonString = messageFormatter.FormatMessage(
                                              err,
                                              "Update Limit Failed, Rollback User Creation",
                                              false,
                                              undefined
                                            );
                                          } else {
                                            if (
                                              ...
                                              crypto.randomBytes(20, function(
                                                err,
                                                buf
                                              ) {
                                                ...
                                                redisClient.set(
                                                  ...
                                                  function(err, val) {
                                                    if (err) {
                                                      jsonString = messageFormatter.FormatMessage(
                                                        err,
                                                        "Create activation token failed",
                                                        false,
                                                        user
                                                      );
                                                      res.end(jsonString);
...
function CreateReportUser(req, res) {
  ...
  Org.findOne({ tenant: tenant, id: company }, function(err, org) {
    if (err) {
      jsonString = messageFormatter.FormatMessage(
        err,
        "Validate Organisation Failed",
        false,
        undefined
      );
      res.end(jsonString);
    } else {
      Console.findOne({ consoleName: req.params.consoleName }, function(
        err,
        appConsole
      ) {
        if (err) {
                ...
                UserAccount.findOne(
                  ...
                  function(err, assignUser) {
                    if (err) {
                      jsonString = messageFormatter.FormatMessage(
                        err,
                        "Validate Assigning User Failed",
                        false,
                        undefined
                      );
                      res.end(jsonString);
                      ...
                      if (
                        ...
                        adminUser.user_meta.role == "superadmin"
                      ) {
                        if (
                          appConsole.consoleUserRoles.indexOf(
                            assignUser.user_meta.role
                          ) > -1
                        ) {
                          var consoleAccessLimitObj = FilterObjFromArray(
                            org.consoleAccessLimits,
                            "accessType",
                            assignUser.user_meta.role
                          );
                          ...
                          if (consoleAccessLimitObj) {
                            var consoleScope = FilterObjFromArray(
                              assignUser.client_scopes,
                              "consoleName",
                              appConsole.consoleName
                            );
                            if (consoleScope) {
                              var menuItem = FilterObjFromArray(
                                consoleScope.menus,
                                "menuItem",
                                req.body.menuItem
                              );
                              if (menuItem) {
                                for (
                                  var j = 0;
                                  j < menuItem.menuAction.length;
                                  j++
                                ) {
                                  var menuAction = FilterObjFromArray(
                                    menuItem.menuAction,
                                    "scope",
                                    req.body.menuAction[j].scope
                                  );
                                  if (menuAction) {
                                    if (req.body.menuAction[j].read) {
                                      menuAction.read =
                                        req.body.menuAction[j].read;
                                    }
                                    if (req.body.menuAction[j].write) {
                                      menuAction.write =
                                        req.body.menuAction[j].write;
                                    }
                                    if (req.body.menuAction[j].delete) {
                                      menuAction.delete =
                                        req.body.menuAction[j].delete;
                                    }
                                ...
                                consoleScope.menus = UniqueObjectArray(
                                  consoleScope.menus,
                                  "menuItem"
                                );
                              }
                            ...
                            UserAccount.findOneAndUpdate(
                              ...
                              function(err, rUser) {
                                if (err) {
                                  ...
                                  jsonString = messageFormatter.FormatMessage(
                                    undefined,
                                    "Update client scope successfully",
                                    true,
                                    undefined
                                  );
                            ...
                            jsonString = messageFormatter.FormatMessage(
                              err,
                              "Access Denied, No Console Access Limit Found",
                              false,
                              undefined
                            );
                            res.end(jsonString);
                          ...
                          jsonString = messageFormatter.FormatMessage(
                            err,
                            "Access Denied, No user permissions",
                            false,
                            undefined
                          );
                          res.end(jsonString);
...
function UpdateUser(req, res) {
  ...
  if (req.params.name) {
              ...
              function(err, userAccount) {
                if (err) {
                  ...
                  if (userAccount) {
                    ...
                    user.multi_login = userAccount.multi_login;
                    user.allowoutbound = userAccount.allowoutbound;
                    user.allowed_file_categories =
                      userAccount.allowed_file_categories;

                    jsonString = messageFormatter.FormatMessage(
                      err,
                      "Update User Account Successful",
                      true,
                      user
                    );
    ...
    jsonString = messageFormatter.FormatMessage(
      new Error("Update User Failed Username empty"),
      "Update User Failed Username empty",
      false,
      undefined
    );
    res.end(jsonString);
    ...
    User.findOne(
      ...
      function(err, myprofile) {
        ...
        if (err) {
          ...
          if (myprofile) {
            myprofile.comparePassword(req.body.oldpassword, function(
              err,
              isMatch
            ) {
              if (!isMatch) {
                jsonString = messageFormatter.FormatMessage(
                  err,
                  "Update User Password Failed No User Found",
                  false,
                  undefined
                );
                res.end(jsonString);
...
function GetMyrProfile(req, res) {
  ...
  try {
    User.findOne({ username: req.user.iss })
      .select("-password")
      .exec(function(err, users) {
        if (err || !users) {
          jsonString = messageFormatter.FormatMessage(
      ...
      if (err) {
        ...
        if (userAccount && userAccount[contact]) {
          ...
          if (contactinfo && contactinfo.contact) {
            ...
            if (contact == "veeryaccount") contactObj.ContactType = "PRIVATE";
          }
        }
...
function UpdateUserProfileEmail(req, res) {
  ...
  User.findOneAndUpdate(
    ...
    { email: { contact: req.params.email, type: "email", verified: false } },
    function(err, users) {
      if (err) {
        jsonString = messageFormatter.FormatMessage(
          err,
  ...
  User.findOneAndUpdate(
    ...
    function(err, users) {
      if (err) {
        jsonString = messageFormatter.FormatMessage(
          err,
          "Update User phone number Failed",
          false,
          undefined
        );
      } else {
        jsonString = messageFormatter.FormatMessage(
          err,
          "Update User phone number Successful",
          true,
          users
        );
      }
...
function RemoveMyUserProfileContact(req, res) {
  ...
  User.findOneAndUpdate(
    ...
    { $pull: { contacts: { contact: req.params.contact } } },
    function(err, users) {
      if (err) {
        jsonString = messageFormatter.FormatMessage(
          err,
...
function UpdateUserProfilePhone(req, res) {
  ...
  User.findOneAndUpdate(
    ...
    {
      phoneNumber: { contact: req.params.email, type: "voice", verified: false }
    },
    function(err, users) {
      if (err) {
        jsonString = messageFormatter.FormatMessage(
...
function SetUserProfileResourceId(req, res) {
  ...
  if (req.params.name) {
    UserAccount.findOneAndUpdate(
      ...
      function(err, users) {
        if (err) {
          jsonString = messageFormatter.FormatMessage(
            err,
            "Update User resource id Failed",
            false,
            undefined
          );
        } else {
    ...
    jsonString = messageFormatter.FormatMessage(
      err,
      "Update User resource id Failed Username empty",
      false,
      undefined
    );
    res.end(jsonString);
                  ...
                  function(err, assignUser) {
                    if (err) {
                      ...
                      if (
                        adminUser &&
                        adminUser.user_meta.role != undefined &&
                        (adminUser.user_meta.role == "admin" ||
                          adminUser.user_meta.role == "supervisor")
                      ) {
                        if (
                          appConsole.consoleUserRoles.indexOf(
                            assignUser.user_meta.role
...
function AddUserScopes(req, res) {
  ...
  Org.findOne({ tenant: tenant, id: company }, function(err, org) {
    if (err) {
      ...
      UserAccount.findOne(
        { user: adminUserName, company: company, tenant: tenant },
        function(err, adminUser) {
          if (err) {
            jsonString = messageFormatter.FormatMessage(
              err,
...
function RemoveUserScopes(req, res) {
  ...
  UserAccount.findOneAndUpdate(
    ...
    { $pull: { user_scopes: { scope: req.params.scope } } },
    function(err, users) {
      if (err) {
        jsonString = messageFormatter.FormatMessage(
          err,
                        ...
                        if (
                          ...
                          if (consoleAccessLimitObj) {
                            ...
                            for (var i in req.body.menuAction) {
                              var userScope = FilterObjFromArray(
                                assignUser.user_scopes,
                                "scope",
                                req.body.menuAction[i].scope
                              );
                              if (userScope) {
                                if (
                                  req.body.menuAction[i].read &&
                                  (!userScope.read || userScope.read == false)
                                ) {
                                  userScope.read = req.body.menuAction[i].read;
                                }
                                if (
                                  req.body.menuAction[i].write &&
                                  (!userScope.write || userScope.write == false)
                                ) {
                                  userScope.write =
                                    req.body.menuAction[i].write;
                                }
                                if (
                                  req.body.menuAction[i].delete &&
                                  (!userScope.delete ||
                                    userScope.delete == false)
                                ) {
                                  userScope.delete =
                                    req.body.menuAction[i].delete;
                                }
    ...
    function(err, adminUser) {
      if (err) {
        ...
        UserAccount.findOne(
          {
            $and: [
              { "client_scopes.consoleName": req.params.consoleName },
              {
                user: req.params.username,
                company: company,
                tenant: tenant
                ...
                UserAccount.findOneAndUpdate(
                  ...
                  function(err, users) {
                    if (err) {
                      jsonString = messageFormatter.FormatMessage(
                        err,
                        "Remove Navigation scope Failed",
                        false,
                        undefined
                      );
                    } else {
...
function GetUserMeta(req, res) {
  ...
  var jsonString;
  UserAccount.findOne(
    { user: req.params.name, company: company, tenant: tenant },
    function(err, users) {
      if (err) {
        jsonString = messageFormatter.FormatMessage(
          err,
...
function GetMyAppScopesByConsole(req, res) {
  ...
  UserAccount.findOne(
    { user: user, company: company, tenant: tenant },
    function(err, users) {
      if (err) {
        jsonString = messageFormatter.FormatMessage(
          err,
...
function GetUserTags(req, res) {
  ...
  UserTag.find({ company: company, tenant: tenant }).exec(function(
    errTags,
    userTags
  ) {
    if (errTags) {
...
function AddFileCategoryToUser(req, res) {
  ...
  DbConn.FileCategory.findOne({ where: [{ Category: req.params.category }] })
      ...
      if (resCat) {
        if (req.user.iss) {
          UserAccount.findOneAndUpdate(
            ...
            { $push: { allowed_file_categories: req.params.category } },
            function(err, users) {
              if (err) {
                jsonString = messageFormatter.FormatMessage(
                  err,
                  "Add file category to User Failed",
                  false,
                  undefined
                );
              } else {
                if (users) {
                  jsonString = messageFormatter.FormatMessage(
                    err,
                    "Add file category to User Successful",
                    true,
                    users
                  );
                } else {
          ...
          jsonString = messageFormatter.FormatMessage(
            new Error("Add file category to User :- Username empty"),
            "Add file category to User :- Username empty",
            false,
            undefined
          );
          res.end(jsonString);
        ...
        jsonString = messageFormatter.FormatMessage(
          new Error("Invalid file category  : " + req.params.category),
          "Invalid file category " + req.params.category,
          false,
          undefined
        );
        res.end(jsonString);
      ...
      if (resCat) {
        if (req.user.iss) {
          UserAccount.findOneAndUpdate(
            ...
            { $pull: { allowed_file_categories: req.params.category } },
            function(err, users) {
              if (err) {
                jsonString = messageFormatter.FormatMessage(
                  err,
...
function GetFileCategories(req, res) {
  ...
  if (req.user.company && req.user.tenant) {
    ...
    DbConn.FileCategory.findAll({
      where: [{ Visible: true }, { Company: company }, { Tenant: tenant }]
    })
      .then(function(resCat) {
        if (resCat) {
          jsonString = messageFormatter.FormatMessage(
          ...
          jsonString = messageFormatter.FormatMessage(
            new Error("No fule categories found"),
            "No fule categories found ",
            false,
            undefined
          );
          res.end(jsonString);
      ...
      .catch(function(errCat) {
        jsonString = messageFormatter.FormatMessage(
          errCat,
          "Error in searching file categories",
          false,
          undefined
        );
        res.end(jsonString);
    ...
    jsonString = messageFormatter.FormatMessage(
      new Error("No Company Tenant details found"),
      "No Company Tenant details found",
      false,
      undefined
    );
    res.end(jsonString);
              ...
              User.findOne(
                ...
                function(err, existingUser) {
                  ...
                  if (!existingUser) {
                    ...
                    jsonString = messageFormatter.FormatMessage(
                      err,
                      "User already in deactivate state",
                      false,
                      undefined
                    );
                    res.end(jsonString);
  ...
  UserAccount.findOne(
    ...
    function(err, user) {
      if (err) {
        ...
        if (user) {
          Org.findOne({ id: user.company, tenant: user.tenant }, function(
            errOrg,
            resOrg
          ) {
            if (errOrg) {
              jsonString = messageFormatter.FormatMessage(
                errOrg,
                "Get organisation details failed",
                false,
                undefined
              );
              res.end(jsonString);
...
function userIsAllowToOutbound(req, res) {
  ...
  if (req.params.Action === "OutboundMode") {
    ...
    UserAccount.findOne(query).exec(function(err, users) {
      if (err) {
        jsonString = messageFormatter.FormatMessage(
          err,
          "Get User Failed",
        ...
        jsonString = messageFormatter.FormatMessage(
          err,
          "Get User Successful",
          users ? users.allowoutbound == true : false,
          null
        );
      }
...
function UserAcccountActivation(req, res) {
  ...
  if (req.params.username) {
    UserAccount.findOneAndUpdate(
      { company: company, tenant: tenant, user: req.params.username },
      {
        active: Active,
        update_at: Date.now()
      },
      function(err, rUser) {
        if (err) {
          jsonString = messageFormatter.FormatMessage(
            err,
            "Update User Account Activation failed",
            false,
            undefined
          );
          res.end(jsonString);
          ...
          jsonString = messageFormatter.FormatMessage(
            undefined,
            "Update User Account Activation succeeded",
            true,
            invitation
          );
          res.end(jsonString);
    ...
    jsonString = messageFormatter.FormatMessage(
      new Error("No username received"),
        ...
        var users = userAccounts.map(function(userAccount) {
          if (userAccount.veeryaccount) {
            ...
            UserAccount.findOneAndUpdate(
              ...
              { veeryaccount: userAccount.veeryaccount },
              function(err, users) {
                if (err) {
                  console.error(err);
                } else {
                  console.log(
                    "Domain Updated...." + userAccount.veeryaccount.contact
                  );
                }
              }
            );
