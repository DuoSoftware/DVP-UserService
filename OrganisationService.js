...
function AddOrUpdateAbandonCallRedialConfig(req, res){
    ...
    AbandonCall.findOne({tenant: tenant, company: company}, function (err, abdConfig) {
        ...
        if(abdConfig)
        {
            AbandonCall.findOneAndUpdate({tenant: tenant, company: company}, {redialCampaignId: req.body.redialCampaignId, redialTime: req.body.redialTime, camScheduleId: req.body.camScheduleId, categoryId: req.body.categoryId, updated_at: Date.now()}, function (err, abandConfig) {
                if(abandConfig)
                {
