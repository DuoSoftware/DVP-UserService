...
function GetMyReceivedInvitations(req, res) {
  ...
  UserInvitation.find({ to: user, status: "pending" }, function(
    err,
    invitations
  ) {
    if (err) {
            ...
            User.findOne(
              ...
              function(err, user) {
                if (err) {
                  jsonString = messageFormatter.FormatMessage(
                    err,
                    "User Invitation failed due to no user found",
                    true,
                    undefined
                  );
                  res.end(jsonString);
                      ...
                      if (err) {
                        ...
                        userAccount.save(function(err, account) {
                          if (err) {
                            ...
                            jsonString = messageFormatter.FormatMessage(
                              undefined,
                              "User Invitation saved successfully",
                              true,
                              invitation
                            );
                            res.end(jsonString);
  ...
  Org.findOne({ tenant: tenant, id: company }, function(err, org) {
    if (err) {
      ...
      if (org) {
        ...
        if (limitObj) {
          if (
            limitObj.accessLimit - limitObj.currentAccess.length >
            to.length
          ) {
            User.find(
              {
              ...
              function(err, users) {
                if (err) {
                  ...
                  if (users && Array.isArray(users)) {
                    ...
                    var userAccounts = users.reduce(function(
                      total,
                      val,
                      index
                    ) {
                    ...
                    UserInvitation.insertMany(invites, function(
                      err,
                      invitations
                    ) {
                      if (err) {
                        ...
                        UserAccount.insertMany(userAccounts, function(
                          err,
                          accounts
                        ) {
                          if (err) {
...
function AcceptUserInvitation(req, res) {
  ...
  UserInvitation.findOneAndUpdate(
    ...
    { status: "accepted", update_at: Date.now() },
    function(err, invitation) {
      if (err) {
        jsonString = messageFormatter.FormatMessage(
          err,
          "Update User Invitation Failed",
          false,
          undefined
        );
        res.end(jsonString);
        ...
        if (invitation) {
          UserAccount.findOneAndUpdate(
            { company: company, tenant: tenant, user: to },
            {
              verified: true,
              joined: Date.now(),
              update_at: Date.now()
            ...
            function(err, rUser) {
              if (err) {
                ...
                jsonString = messageFormatter.FormatMessage(
                  undefined,
                  "Update User Invitation Successful",
                  true,
                  invitation
                );
                SenNotification(
                  ...
                  "system",
                  invitation.from,
                  "User " + invitation.to + " Accepted Your Request",
                  function() {}
                );
                res.end(jsonString);
              }
...
function RejectUserInvitation(req, res) {
  ...
  UserInvitation.findOneAndUpdate(
    ...
    { status: "rejected", update_at: Date.now() },
    function(err, invitation) {
      if (err) {
        jsonString = messageFormatter.FormatMessage(
          err,
            ...
            function(err, account) {
              if (err) {
                ...
                SenNotification(
                  ...
                  "system",
                  invitation.from,
                  "User " + invitation.to + " Rejected Your Request",
                  function() {}
                );
                res.end(jsonString);
              }
...
function CancelUserInvitation(req, res) {
  ...
  UserInvitation.findOneAndUpdate(
    ...
    { status: "canceled", update_at: Date.now() },
    function(err, invitation) {
      if (err) {
        jsonString = messageFormatter.FormatMessage(
          err,
...
function ResendUserInvitation(req, res) {
  ...
  UserInvitation.findOneAndUpdate(
    ...
    { status: "pending", update_at: Date.now() },
    { new: true },
    function(err, invitation) {
      if (err) {
        jsonString = messageFormatter.FormatMessage(
...
var SenNotification = function(company, tenant, from, to, message, callback) {
  ...
  if (config.Services.dynamicPort || validator.isIP(config.Services.notificationServiceHost)) {
    serviceurl = util.format(
      "http://%s:%s/DVP/API/%s/NotificationService/Notification/initiate",
      config.Services.notificationServiceHost,
...
function RequestRabbitMQ(req, res) {
  PublishToQueue("TEST", {
    from: "userservice",
    to: "amqp",
    test: "test"
...
module.exports.RequestRabbitMQ = RequestRabbitMQ;
